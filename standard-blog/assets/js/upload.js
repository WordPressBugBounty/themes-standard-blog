...
jQuery(function($){
  ...
  jQuery(document).on('click', 'input.select-img', function( event ){
    ...
    var Standard_Blog_Image = wp.media.controller.Library.extend({
      defaults :  _.defaults({
    ...
    standard_blog_file_frame = wp.media.frames.standard_blog_file_frame = wp.media({
      button: {
        text: jQuery( this ).data( 'uploader_button_text' ),
      },
      state : 'author-info-insert-image',
      states : [
        new Standard_Blog_Image()
        ],
          multiple: false  // Set to true to allow multiple files to be selected
        });

    // When an image is selected, run a callback.
    standard_blog_file_frame.on( 'select', function() {

      var state = standard_blog_file_frame.state('author-info-insert-image');
