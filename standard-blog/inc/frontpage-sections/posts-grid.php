...
// Posts Grid Section.
$posts_grid_section = get_theme_mod( 'standard_blog_posts_grid_section_enable', false );
...
$posts_grid_content_type = get_theme_mod( 'standard_blog_posts_grid_content_type', 'post' );

if ( $posts_grid_content_type === 'post' ) {

	for ( $i = 1; $i <= 3; $i++ ) {
		$content_ids[] = get_theme_mod( 'standard_blog_posts_grid_post_' . $i );
	}
	...
	$cat_content_id = get_theme_mod( 'standard_blog_posts_grid_category' );
	$args           = array(
		'cat'            => $cat_content_id,
		'posts_per_page' => absint( 3 ),
	);
...
if ( $query->have_posts() ) {
	...
	$section_title    = get_theme_mod( 'standard_blog_posts_grid_title', __( 'Latest Posts', 'standard-blog' ) );
	$viewall_btn      = get_theme_mod( 'standard_blog_posts_grid_view_all_button_label', __( 'View All', 'standard-blog' ) );
	$viewall_btn_link = get_theme_mod( 'standard_blog_posts_grid_view_all_button_url', '#' );
	?>
	<div id="standard_blog_posts_grid_section" class="frontpage post-grid-section three-column">
