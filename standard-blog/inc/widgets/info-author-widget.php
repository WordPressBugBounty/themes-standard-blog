<?php
if ( ! class_exists( 'Standard_Blog_Author_Info_Widget' ) ) {
	/**
	 * Adds Standard Blog Author Info Widget.
	 */
	class Standard_Blog_Author_Info_Widget extends WP_Widget {

		/**
		 * Register widget with WordPress.
		 */
		public function __construct() {
			$standard_blog_author_info_widget = array(
				'classname'   => 'widget adore-widget author-widget',
				'description' => __( 'Retrive Author Information Widget', 'standard-blog' ),
			);
			parent::__construct(
				'standard_blog_author_info_widget',
				__( 'Adore Widget: Author Info Widget', 'standard-blog' ),
				$standard_blog_author_info_widget
			);
		}
		...
		public function form( $instance ) {
			...
			?>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Section Title:', 'standard-blog' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'name' ) ); ?>"><?php esc_html_e( 'Author Name:', 'standard-blog' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'designation' ) ); ?>"><?php esc_html_e( 'Author Designation:', 'standard-blog' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'description' ) ); ?>"><?php esc_html_e( 'Description :', 'standard-blog' ); ?></label>
			...
			<div>
				<label for="<?php echo esc_attr( $this->get_field_id( 'author_image_url' ) ); ?>"><?php esc_html_e( 'Author Image URL', 'standard-blog' ); ?></label>:<br />
				...
				<div class="image-preview" style="<?php echo esc_attr( $style_attr ); ?>">
					<img src="<?php echo esc_url( $image_url ); ?>" alt="<?php esc_attr_e( 'Preview', 'standard-blog' ); ?>" style="width: 200px;"/>
				</div><!-- .image-preview -->
				<input type="button" class="select-img button" value="<?php esc_attr_e( 'Upload', 'standard-blog' ); ?>" data-uploader_title="<?php esc_attr_e( 'Select Image', 'standard-blog' ); ?>" data-uploader_button_text="<?php esc_attr_e( 'Choose Image', 'standard-blog' ); ?>" style="margin-top:5px;" /><br/>
			</div>
			<?php
			for ( $i = 1; $i <= 4; $i++ ) {
				...
				?>
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'link-' . $i ) ); ?>"><?php echo sprintf( esc_html__( 'Social Link %d :', 'standard-blog' ), $i ); ?></label>
 ...
 *
 * @since Standard Blog 1.0
 **/
function standard_blog_author_info_widget_image_upload_enqueue( $hook ) {
	...
	wp_enqueue_media();
	wp_enqueue_script( 'author-info-widget-image-upload-script', get_template_directory_uri() . '/assets/js/upload.js', array( 'jquery' ), STANDARD_BLOG_VERSION, true );
}

add_action( 'admin_enqueue_scripts', 'standard_blog_author_info_widget_image_upload_enqueue' );
